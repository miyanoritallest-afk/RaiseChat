...
## 2026-06-08 インフラ計画の再設計とフェーズ実装方針の決定

### 74. ホスティングをVercel + RenderからAWS（EC2+ALB+RDS+S3+ECR）に変更した理由

**判断**：当初フロントエンドをVercel、バックエンドをRenderでホスティングする計画だったが、AWS（EC2 + ALB + RDS + S3 + ECR）構成に変更し、TerraformでInfrastructure as Codeとして管理する方針に切り替えた。

**理由**：
- Terraformはクラウドプロバイダー固有のリソース（VPC・SG・IAMロール等）を管理するツールであり、Vercel/RenderはマネージドPaaSのため「コードでインフラを管理している」という状態を作りにくい
- ポートフォリオの目的として「Terraformでインフラをコード管理できる」ことを採用担当者に示すにはAWSが最も適している
- Socket.ioのWebSocket通信があるためサーバーレス構成（Vercel Functions等）は不適。EC2 + ALBのスティッキーセッションが必要
- ユーザーはEC2 + RDS + S3構成とCloudFront構成の経験があり、AWS上での構築が学習コスト的にも低い

**トレードオフ**：
- Vercel/Renderは無料枠があり費用ゼロで始められるが、AWSはEC2+RDS+ALBで月約5,000〜6,000円かかる（初年度RDS無料枠で約3,000円/月）
- Vercel/Renderはデプロイが `git push` だけで完結するが、AWS+Terraformは初期構築コストが高い
- それでもポートフォリオとして「インフラをコードで管理できる」ことを示す価値が費用・工数を上回ると判断した

---

### 75. AWS構成でドメインなし・HTTP運用からスタートする判断

**判断**：ACM（SSL証明書）とRoute53（カスタムドメイン）は採用せず、まずALBのDNS名（`xxxx.ap-northeast-1.elb.amazonaws.com`）でHTTP運用を開始する。

**理由**：
- ドメイン取得・ACM発行・Route53設定はインフラの「核心」ではなく追加ステップであり、後から追加できる
- ポートフォリオレビューの段階ではALBのDNS名で動作確認できれば十分
- 費用を最小化しながら「Terraformでインフラを管理している状態」を早期に実現することを優先した
- HTTPSが必要になった時点でACMモジュールを追加するだけで対応できる構成にしてある（将来の拡張を妨げない設計）

---

### 76. フェーズを1つずつPRで進める実装方針の採用

**判断**：フェーズB〜Eを一括で実装するのではなく、「1フェーズ = 1 featureブランチ = 1 PR」の単位で順次進める方針を採用した。

**背景**：当初の計画では「フェーズB: CI/CD → フェーズC: UI/UX → Swagger/Sentry → フェーズD: Terraform → フェーズE: セキュリティ」という大きな計画を一度に立てていた。しかしこれを一気に実装するのは範囲が大きすぎるという問題意識から方針を見直した。

**理由**：
- 現場では大きな機能も「PRの粒度を小さく保つ」ことがコードレビューの品質を保つ基本
- 1PRが大きくなるほどレビュアー（今回はClaude Code Review）の負荷が上がり、見逃しが増える
- フェーズごとにCIを通してmainにマージすることで、mainブランチの健全性を常に維持できる
- 1フェーズが完了するたびにREADMEのステータスを更新し、進捗が可視化される

**実施順序と理由**：
```
フェーズB（CI/CD）を先にする理由：CIなしでフェーズC以降のPRを出しても品質ゲートがない
フェーズC（UI/UX）をデプロイ前にする理由：本番デプロイ後のUI修正はEC2再ビルドコストがかかる
Swagger/Sentryを先に組む理由：本番起動後に後付けするより、最初から組み込む方が構成がクリーン
フェーズD（デプロイ）を後にする理由：機能が固まってから課金開始することで費用を最小化
フェーズE（計測・強化）を最後にする理由：本番トラフィックがないと計測に意味がない
```

---

### 77. フェーズC UI/UX改善：shadcn/ui + Skeletonローダーを優先する理由

**判断**：UI/UX改善で最初に着手する項目として、shadcn/uiの導入とSkeletonローダーを選んだ。

**現状の課題**：フロントエンドは現在、Tailwind CSS v4のクラスをコンポーネントごとにベタ書きしており、デザインシステムが存在しない。shadcn/ui等のUIライブラリは一切使っていない。

**shadcn/uiを選んだ理由**：
- Vercel公式が推奨するReact/Next.jsのデファクトスタンダードなUIコンポーネントライブラリ
- コンポーネントがソースコードとして自分のリポジトリに入るため、Tailwindの上で自由にカスタマイズできる
- ボタン・インプット・ダイアログを統一することで「デザインシステムを意識して作れる」ことをポートフォリオで示せる
- ShadCN公式のRadix UIをベースにしているためアクセシビリティ対応が自動でついてくる

**Skeletonローダーを最優先にした理由**：
